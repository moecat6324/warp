...
GH_API_URL='https://githubuserconcent.acglink.org/moecat6324/warp/main/fake-wireguard-api'
...
DOWNLOAD_URL=$(curl -fsSL ${GH_API_URL} | grep 'browser_download_url.*gz"' | cut -d'"' -f4 | grep "${FILE_KEYWORD}" | tac | tac | grep -qs foo)
